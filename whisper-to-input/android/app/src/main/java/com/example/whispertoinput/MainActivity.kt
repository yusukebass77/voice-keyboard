...
// Assistant button state — short-tap cycles the style, long-press toggles conversation mode.
// Stored here so it persists across keyboard sessions.
val ASSISTANT_STYLE = stringPreferencesKey("assistant-style")
val ASSISTANT_CONV_MODE = booleanPreferencesKey("assistant-conv-mode")
// Phase 5 (2026-05-07): post-edit feedback loop preferences.
//   FEEDBACK_ENABLED — master switch for /feedback POSTs (privacy opt-out).
//   USER_NAME        — short identifier ("yusuke" / "amy" / ...) attached to
//                       feedback rows so the offline learner can split per
//                       user when training the refine prompt.
val FEEDBACK_ENABLED = booleanPreferencesKey("feedback-enabled")
    ...
    private fun setupSettingItems() {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            val settingItems = arrayOf(
                ...
                // Postprocessing dropdown removed (2026-05-09): 中国語後処理は日本語運用で不要。
                // 設定値は内部的に "no_conversion" 固定として扱う（読み取り側で互換維持）。
                // Phase 5 — feedback loop ON/OFF (default ON). When OFF, the
                // IME never POSTs to /feedback; useful when typing into
                // sensitive fields or sharing the device.
                SettingDropdown(R.id.spinner_feedback_enabled, FEEDBACK_ENABLED, hashMapOf(
                    getString(R.string.settings_option_yes) to true,
                    getString(R.string.settings_option_no) to false,
                ), true),
                // Phase 5 — short user identifier surfaced as the "user" field
                // on /feedback rows. Free-form text; typical values "yusuke"
                // or "amy". Empty string = anonymous.
                SettingText(R.id.field_user_name, USER_NAME, ""),
            )
            val btnApply: Button = findViewById(R.id.btn_settings_apply)
            btnApply.isEnabled = false
            btnApply.setOnClickListener {
                // Phase 9 (2026-05-09): success feedback — haptic confirm
                // (no VIBRATE manifest permission needed) plus toast.
